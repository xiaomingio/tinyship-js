...
import { dryRunHost } from '../dist/deploy.js';
...
test('dry-run host prints the same deploy commands without executing them', async () => {
  ...
  assert.ok(logs.some(line => line.includes('# Dry run web to root@example.com:/var/www/example')));
  assert.equal(logs[0], '');
  assert.ok(logs.some(line => line.startsWith('# [1/5] validate env files')));
  assert.ok(logs.some((line, index) => line.startsWith('# [3/5] prepare remote directories') && logs[index - 1] === ''));
  ...
  assert.ok(logs.some(line => line.includes('dist package.json package-lock.json ecosystem.config.cjs tinyship.config.yml .env.prod.example-server')));
